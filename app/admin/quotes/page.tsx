...
async function getQuotes(status?: string, search?: string, sort = 'event_date', order = 'asc', page = 1) {
    ...
    let query = db.from('quotes')
        .select('id, token, status, client_name, client_lastname, client_email, event_date, total_price, created_at, comuna_name, comuna_other, dispenser, service_type', { count: 'exact' });
