...
export const ConfirmationEmail: React.FC<Readonly<ConfirmationEmailProps>> = ({ quote, isAdmin = false }) => {
  ...
  if (isAdmin) {
    return (
      <BaseLayout preview={`[${isDirect ? 'Pedido Confirmado' : 'Reserva Confirmada'}] ${clientName} - ${eventDate}`} accentColor={greenColor}>
        <h2 style={{ color: greenColor, margin: '0 0 8px', fontSize: '20px', lineHeight: '1.3' }}>✅ {isDirect ? 'Pedido pagado y agendado' : 'Reserva confirmada por el cliente'}</h2>
        <Text style={{ color: gray, margin: '0 0 24px', fontSize: '14px' }}>
          <strong>{clientName}</strong> ha confirmado su {isDirect ? 'pedido' : 'reserva'} para el {eventDate}.
        </Text>
  ...
  return (
    <BaseLayout preview={`✅ ${isDirect ? 'Pedido confirmado' : 'Reserva confirmada'} – ${eventDate}`} accentColor={greenColor}>
      <h2 style={{ color: greenColor, margin: '0 0 8px', fontSize: '22px', lineHeight: '1.3' }}>✅ ¡{isDirect ? 'Pedido confirmado!' : 'Reserva confirmada!'}</h2>
      <Text style={{ color: gray, margin: '0 0 24px', fontSize: '15px', lineHeight: '1.6' }}>
        Hola <strong>{clientName}</strong>, tu {isDirect ? 'pedido' : 'reserva'} para el <strong>{eventDate}</strong> ha sido completado. {isDirect ? 'Hemos recibido tu pago y el despacho ya ha quedado agendado.' : 'Para asegurar la fecha, realiza el abono del 50%.'}
      </Text>
